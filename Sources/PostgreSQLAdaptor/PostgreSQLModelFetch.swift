...
open class PostgreSQLModelFetch: AdaptorModelFetch {
                ...
                -> [ Entity ]
  {
    ...
    // Positional subscripts because we do not privide a schema and
    // PCK doesn't provide result reflection.
    let tableColSubscript = 0 // "table"
    let nameColSubscript  = 1 // "name"
    ...
    let constraintNameColIdx    = 0
    let sourceTableColIdx       = 1
    let sourceColumnColIdx      = 2
    let sourceColumnCountColIdx = 3
    let foreignTableNameColIdx  = 4
    let targetColumnColIdx      = 5
    let constraintTypeColIdx    = 6
    //let onUpdateColIdx          = 7
    let onDeleteColIdx          = 8
    //let matchTypeColIdx         = 9
    ...
    let recordsByTable : [ String : [ AdaptorRecord ] ] = {
      ...
      for record in attributeRecords {
        guard let key = record[tableColSubscript] as? String else { continue }
    ...
    let pkeysByTable : [ String : [ String ] ] = {
      ...
      for record in pkeyRecords {
        guard let key  = record[tableColSubscript] as? String else { continue }
        guard let pkey = record[nameColSubscript]  as? String else {
          continue
        }
    ...
    let autoIncrByTable : [ String : Set<String> ] = {
      ...
      for record in autoIncrRecords {
        guard let key  = record[tableColSubscript] as? String else { continue }
        guard let pkey = record[nameColSubscript]  as? String else { continue }
    ...
    let fkeysByTable : [ String : [ AdaptorRecord ] ] = {
      ...
      for record in foreignKeyRecords {
        guard let key = record[sourceTableColIdx] as? String else { continue }
        if case nil = grouped[key]?.append(record) {
          grouped[key] = [ record ]
        }
      }
    ...
    for table in tables {
      ...
      if let foreignKeyRecords = fkeysByTable[table] {
        let fkeysByConstraint : [ String : [ AdaptorRecord ] ] = {
          var grouped = [ String : [ AdaptorRecord ] ]()
          for record in foreignKeyRecords {
            guard let key = record[constraintNameColIdx] as? String else {
              continue
            }
            if case nil = grouped[key]?.append(record) {
              grouped[key] = [ record ]
        ...
        for ( name, fkeys ) in fkeysByConstraint {
          ...
          for fkey in fkeys {
            ...
            if let constraintType = fkey[constraintTypeColIdx] as? String {
            ...
            if let count = fkey[sourceColumnCountColIdx] as? Int, count > 1 {
              // TODO: Not too hard to add, but then pretty rare as well :-)
              log.warn("unsupported multi-column foreign-key constraint:", fkey)
              continue
            }
            
            guard let destname     = fkey[foreignTableNameColIdx] as? String,
                  let sourceColumn = fkey[sourceColumnColIdx]     as? String,
                  let targetColumn = fkey[targetColumnColIdx]     as? String
             else { continue }
            ...
            if let deleteRule = fkey[onDeleteColIdx] as? String {
              switch deleteRule {
  ...
  func _fetchPGPrimaryKeyNamesOfTable(_ table: String) throws -> [ String ] {
    ...
    let pkeyColIdx = 0 // "pkey"
    var pkeys = [ String ]()
    try channel.querySQL(sql) { record in
      if let pkey = record[pkeyColIdx] as? String {
        pkeys.append(pkey)
      }
    }
    return pkeys
       ...
       -> [ Attribute ]
  {
    ...
    for colinfo in columnInfos {
      guard let colname = colinfo[colnameColIdx] as? String else { continue }
      
      let exttype = (colinfo[exttypeColIdx] as? String)?.uppercased()
      ...
      if let lens = colinfo[attlenColIdx] as? String, let len = Int(lens),
         len > 0
      {
        attribute.width = len
      }
      
      if let nulls = colinfo[attnotnullColIdx] as? String {
...
extension AdaptorChannel { // Utility

  /* utility */
  
  func fetchSingleStringRows(_ _sql: String, column: Int) throws -> [ String ] {
    var values = [ String ]()
